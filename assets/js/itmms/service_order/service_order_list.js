    ...
    obj.ajax_delete_service_order_by_id = function( $small_modal ) {
        ...
        $.ajax({
            ...
            success : function( result ) {
                if( result.status ) {
                    obj.service_order_list.ajax.reload();
                    toastr.success( 'Removed!', "itmms | Service Order" );

                    $small_modal.modal( 'hide' );
                }
                else {
                    toastr.error( 'Unable to delete', " itmms | Service Order" );
                }
            },
    ...
    obj.ajax_mark_open_service_order_by_id = function( $small_modal ) {
        ...
        $.ajax({
            ...
            success : function( result ) {
                if( result.status ) {
                    obj.service_order_list.ajax.reload();
                    toastr.success( 'Marked as OPEN!', "itmms | Service Order" );

                    $small_modal.modal( 'hide' );
                }
                else {
                    toastr.error( 'Unable to mark as open', " itmms | Service Order" );
                }
            },
            error : function( xhr, status ) {
                alert( xhr.responseText );
