$( function( $ ) {
    ...
    set_report_title('Cluster ', obj.date_from, obj.date_to);

    $('[data-role="date_filter"]').click(function(e){
        obj.date_from = $('#date_from').val();
        obj.date_to = $('#date_to').val();
        obj.cluster_report.ajax.reload();
        set_report_title('Cluster ', obj.date_from, obj.date_to);
    });
    ...
    $( document ).on( 'click', '[data-bind]', function(e){
        ...
        var url = $( this ).data( 'bind' ),
            cluster_code = $( this ).data( 'id' ),
            method = $( this ).data( 'method' );

            load_modal( url, cluster_code, method );
    } );

    obj.cluster_report = $('#cluster-report').DataTable({
        'processing': true,
        'serverSide': true,
        'responsive': true,
        'paging': false,
        ...
        'ajax': {
            'url' : 'ajax_cluster/get_cluster_report_for_table',
            'data': function ( d ) {
                        ...
                        <div class="text-center"> \
                            <div class="btn-group"> \
                                ...
                                <ul class="dropdown-menu pull-right" role="menu"  style="padding: 0 0 7px 0"> \
                                    ...
                                    <li> \
                                        <a href="javascript:void(0)" data-method="view" data-bind="view_report_details" data-id="' + row.cluster_code + '"><i class="fa fa fa-external-link fa-fw fg-cyan"></i> View Details</a> \
                                    </li> \
                                </ul> \
                            </div> \
                        </div> \
                    ';
                    if(row.cluster_code !== 'OVERALL TOTAL'){
                        return html;
                    }else{
                        return null;
                    }
    ...
    function ajax_get_cluster_report_details( $modal ) {
        ...
        $('[data-role="complaint_type"]').click(function(e){
            ...
            obj.cluster_report_details.ajax.reload();
        });

        obj.cluster_report_details = $('#report-details').DataTable({
            'processing': true,
            'serverSide': true,
            'responsive': true,
            'ajax': {
                'url' : 'ajax_service_order/get_report_details_for_table/' + obj.cluster_code,
                'data': function ( d ) {
    ...
    function load_modal( url, cluster_code, method ) {
        var $modal = $( '#large' ),
            ajax_url = 'modal/' + url;

            obj.method = method;
            obj.cluster_code = cluster_code;

        switch( obj.method ){
            case 'view':
                obj.ajax_get_modal_content( ajax_url, $modal ).done( function(){
                    ajax_get_cluster_report_details( $modal );
                    $modal.modal('show');
                } );
                break;
            default :
