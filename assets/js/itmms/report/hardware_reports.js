$( function( $ ) {
    ...
    set_report_title('Hardware ', obj.date_from, obj.date_to);

    $('[data-role="date_filter"]').click(function(e){
        obj.date_from = $('#date_from').val();
        obj.date_to = $('#date_to').val();
        obj.hardware_reports.ajax.reload();
        set_report_title('Hardware ', obj.date_from, obj.date_to);
    });
    ...
    obj.hardware_reports = $('#hardware-report').DataTable({
        'processing': true,
        'serverSide': true,
        'responsive': true,
        'paging': false,
        ...
        'ajax': {
            'url' : 'ajax_service_order/get_hardware_reports',
            'data': function ( d ) {
    ...
    function ajax_get_hardware_report_details( $modal ) {
        ...
        switch( obj.method ){
            case 'view':
                obj.ajax_get_modal_content( ajax_url, $modal ).done( function(){
                    ajax_get_hardware_report_details( $modal );
                    $modal.modal('show');
                } );
                break;
            default :
