...
class Cluster_Model extends MY_Model
{
    ...
    public function get_cluster_details_by_cluster_code($cluster_code)
    {
        $query = $this->db->select('*')
            ->from('clusters')
    ...
    public function get_clusters($data, $details = false)
    {
        ...
        $sql = 'SELECT r.room_id, r.room_no, clr.cluster_id, clr.cluster_code, clr.cluster_name, clr.type, cd.designation, COUNT(DISTINCT cr.classroom_id) as num_classrooms ';
        $sql .= 'FROM clusters clr ';
        $sql .= 'INNER JOIN rooms r ON clr.room_id = r.room_id ';
        $sql .= 'LEFT JOIN classrooms cr ON clr.cluster_id = cr.cluster_id  ';
        $sql .= 'LEFT JOIN computer_designation cd ON clr.cluster_code = cd.designation ';
        $sql .= 'WHERE 1 ';
        ...
        $sql .= 'GROUP BY r.room_id, r.room_no, clr.cluster_id, clr.cluster_code, clr.cluster_name, clr.type, cd.designation ';
